...
back_server="http://35.202.112.126:3000"
...
def atraccion(request, nombre):
    res = requests.get('http://35.202.112.126:3000/api/attractions/'+nombre)
...
def registrar(request):
    ...
    res = requests.post("http://35.202.112.126:3000/api/users/turn", json=params)
    response = json.loads(res.text)
    print(response)
    print("hola")
...
def usar_t(request):
    res_user = requests.get('http://35.202.112.126:3000/api/users/'+request.POST["id"])
    res_user = json.loads(res_user.text)
    res_attraction = requests.get('http://35.202.112.126:3000/api/attractions/'+request.POST["attr"])
    ...
    if(int(res_user['Attraction']) == int(request.POST["attr"])and (res_user['Turn'] >= res_attraction['CurrentRoundTurn'] and res_user['Turn'] <= res_attraction['CurrentRoundTurn'] +res_attraction['Capacity'])):
        print("paaas")
        requests.put("http://35.202.112.126:3000/api/users/"+ request.POST["id"] +"/removeturn")
        requests.post("http://35.202.112.126:3000/api/users/"+ request.POST["id"] +"/reward?amount=5")
        return redirect('attraction', nombre=request.POST['attr'])
...
def rewards(request):
    global premios2
    res = requests.get('http://35.202.112.126:3000/api/rewards')
...
def reward(request, nombre):
    res = requests.get('http://35.202.112.126:3000/api/rewards/'+nombre)
    response = json.loads(res.text)
    print(response)
...
def buy_reward(request):
    ...
    res = requests.post("http://35.202.112.126:3000/api/users/buyreward", json=params)
    response = json.loads(res.text)
    print(response)
    print("hola")
    if'error' in response:
